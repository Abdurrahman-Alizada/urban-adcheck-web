...
import { useGetRoomByIdQuery, useSendMessageRoutesMutation } from '@/redux/reducers/messages/messagesThunk';
...
function ViewDetails() {
  ...
  const {data:messages}=useGetRoomByIdQuery(params?.jobId);
  console.log("messages",messages)
  const {data:user}=useGetCurrentLoginUserQuery();
  console.log("user",user)
  const [sendMessageRoutes]=useSendMessageRoutesMutation();
  ...
  const handleMessageRoute=()=>{
    try {
      if (messageInput.trim() === '') {
        ...
        sendMessageRoutes({message:messageInput, roomId:params?.jobId})
        .then((res)=>{
         if(res?.data){
          console.log(res);
         }
                  ...
                  <div className='flex flex-col flex-1 overflow-y-auto bg-gray-50 p-4'>
                      {/* Sender message & reciever message*/}
                      {
                          messages?.data?.messages?.map((message,index)=>(
                              <div key={index} className={`mb-2 ${user?._id==message?.sender ? 'text-left': 'text-right'}`}>
                              <span className="block text-black max-w-max">
                                ...
                                <span
                                  className={`block px-3 py-2 rounded-lg max-w-max ${
                                    user?._id === message?.senderId ? 'bg-primary text-white' : 'bg-gray-300 text-black'
                                  }`}
                                >
                                  {message?.message}
                              </span>
                    ...
                    <div className="p-4 border-t border-gray-200 flex gap-2 items-center">
                      ...
                      />
                      {/* send button */}
                       <BsFillSendCheckFill size={20} className="w-12 h-10 p-1 bg-secondary text-white rounded-lg hover:bg-primary"
