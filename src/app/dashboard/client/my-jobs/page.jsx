                  ...
                  {activeRow === job?._id && (
                    <div className="absolute top-25 right-[10%] z-10 w-[200px] bg-white shadow-lg rounded-md">
                      <div className="flex items-center gap-2 hover:bg-[#E8F7FF] p-2 cursor-pointer">
                        ...
                        <span onClick={()=>router.push(`/dashboard/client/my-jobs/${job?._id}`)} className="text-[14px] text-grayColor">
                          View job details
                        </span>
                        
                      </div>
                      <div  className="flex items-center gap-2 hover:bg-[#E8F7FF] p-2 cursor-pointer" onClick={() => handleMarkAsExpire(job)}>
                        <TiDeleteOutline size={16} color="#767E94" />
                        <span className="text-[14px] text-grayColor">Mark it expire</span>
                      </div>
                      <div onClick={()=>handleDeletePopup(job)} className="flex items-center gap-2 hover:bg-[#E8F7FF] p-2 cursor-pointer">
                        <RiDeleteBin6Line size={16} color="#767E94" />
                        <span className="text-[14px] text-grayColor">Delete job</span>
                      </div>
                    </div>
                  )}
                ...
                {showExpirePopup && (
                  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="bg-white rounded-lg shadow-lg p-6 w-[90%] max-w-md">
                      <h2 className="text-xl font-semibold mb-4">Mark Job as Expire</h2>
                      <p className="text-gray-600 mb-6">
                        Are you sure you want to mark the job <strong>{selectedJob?.jobTitle}</strong> as expired?
                      </p>
                      <div className="flex justify-end gap-4">
                        ...
                        <button
                          onClick={() => {
                            ...
                            console.log(`Job ${selectedJob?.jobTitle} marked as expired.`);
...
{ showDeletePopup && (
                  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="bg-white rounded-lg shadow-lg p-6 w-[90%] max-w-md">
                      <h2 className="text-xl font-semibold mb-4">Delete The Job</h2>
                      <p className="text-gray-600 mb-6">
                        Are you sure you want <strong>{selectedJob?.jobTitle}</strong> to be deleted?
                      </p>
                      <div className="flex justify-end gap-4">
                        <button
                        ...
                        <button
                          onClick={() => {
                            console.log(`Job ${selectedJob?.jobTitle} deleted`);
